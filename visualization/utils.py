...
def convert_stations_specs():
    ...
    chosen.to_csv('./dwd_station_specs_old.csv', index=False)

def convert_stations_specs_new():
    data = pd.read_table('Master.txt', sep='[ ]{2,}')
    chosen = data[['Stations_id', 'geoBreite', 'geoLaenge', 'Stationshoehe', 'Stationsname', 'Bundesland']]
    chosen.columns = ['id', 'lat', 'lon', 'height', 'name', 'state']
    chosen.drop_duplicates(inplace=True)
    chosen.sort_values(by='id', inplace=True)
...
if __name__ == '__main__':
    convert_stations_specs()
    convert_stations_specs_new()
